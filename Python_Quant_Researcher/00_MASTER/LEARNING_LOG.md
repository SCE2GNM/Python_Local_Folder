...
**Donchian Channel Breakout** — Rolling N-day high/low channel. Entry fires when daily close crosses above the upper band (N-day rolling high, prior bar). Exit fires when daily low crosses below the lower band (N-day rolling low) — interpreted as the trend failing to maintain momentum. Mechanically simple: no indicator computation, no smoothing, no lookback ambiguity. The breakout signal is the price itself making a new N-day high. Complementary to mean reversion strategies: Donchian captures the initial breakout move, while RSI/Bollinger would fade the same move. Best BNB result: period=20, stop=5%, SMA-120 filter — post-break PF 2.961. The channel exit (lower band touch) and trailing stop (5% from peak) are redundant-but-complementary: the trailing stop catches trend reversals within the channel; the channel exit catches sustained downward breaks.

Introduced: Week 9. Source: 01_altcoin_discovery_grid.py, 07_bnb_charts.py, RISK_REGISTER_BNB_DONCHIAN.md.

**Pipeline Phase Gate Checklist** — Mandatory pipeline phase table (Phases 0–6) added to every strategy risk register. Each phase must be marked Complete and signed off before the next phase begins. Claude Code must check the phase gate table at the start of every session and refuse to proceed to Phase N+1 work if Phase N is not signed off. If a phase is skipped or deferred, written justification must be recorded in the table before the next phase starts. The gate prevents implicit forward jumps (e.g., starting walk-forward before stability analysis is documented, or starting deployment before red team review). Implemented Week 9 after BNB analysis revealed that several retrospective tests had been run without formal sign-off at each stage. See METHODOLOGY_STANDARDS.md — Pipeline Phase Gate Standard section.

Introduced: Week 9. Source: METHODOLOGY_STANDARDS.md, RISK_REGISTER_BNB_DONCHIAN.md.

**B&H Exception Rule (High-MDD Passive Benchmark)** — When an asset's buy-and-hold maximum drawdown exceeds −60%, the standard benchmark gates (annual return ≥ 2× B&H, Sortino ≥ 1.5× B&H) are both suspended. A passive investor who holds through an 80% drawdown to generate Sortino 1.1 is not a meaningful risk-adjusted benchmark for a risk-managed active strategy. Substitute quality gates when B&H MDD > −60%: (1) Strategy Sortino > 0.8 (absolute, not relative); (2) Strategy MtM MDD better than −50%. The MDD gate is never suspended — a risk-managed strategy must always demonstrate materially better drawdown than passive. First applied to BNB: B&H MDD −80.1%, substitute gates both pass (Sortino 0.843, MDD −29.7%). Documented in METHODOLOGY_STANDARDS.md — B&H Benchmark Filter Standard section.

Introduced: Week 9. Source: METHODOLOGY_STANDARDS.md, RR-BNB-007, BNB Donchian validation pipeline.

**Parameter Optimisation Sequence (Phase 3A–3E)** — Mandatory sequential structure for strategy parameter optimisation: (A) Entry parameters — grid search on entry signal parameters, rank by post-break PF; (B) Exit method comparison — test all candidate exit types (fixed stop, ATR trail, channel exit, EMA exit) with entry parameters fixed; (C) Regime filter test — test SMA-50/100/120/200/EMA-50 as entry gates, compare by post-break PF; (D) Joint optimisation — combined grid of top entry × exit × filter combinations; (E) Monte Carlo — resample from validated parameter set. Prior to this standard, exit and regime filter testing were done ad hoc — sometimes in a different order, sometimes not at all. The mandatory sequence prevents optimisation decisions from being made in the wrong order (e.g., selecting entry parameters before knowing which exit will be paired with them). Implemented in STRATEGY_RESEARCH_PIPELINE.md v2.0 (Week 9). Applied retroactively as gap items for ETH RSI (RR-RSI-008, RR-RSI-009).

Introduced: Week 9. Source: STRATEGY_RESEARCH_PIPELINE.md v2.0, bnb_exit_regime_results.csv.

**Kelly Position Sizing — Correct Formula** — Position size = (f* × capital) / stop%. Never implement as position = f* × capital. The Kelly fraction f* represents the fraction of capital to RISK, not to DEPLOY. At a 5% stop, deploying f* × capital as position size underestimates the correct position by a factor of 1/stop_pct = 20×. Example: f*=13.6%, capital=$150, stop=5%: correct position = ($150 × 0.136) / 0.05 = $408. Incorrect implementation = $150 × 0.136 = $20.40. The position size is large because you are only risking f* × capital (the dollar amount at risk = $408 × 5% = $20.40 = 13.6% of capital). This distinction was first established in Week 6 but continues to be the most common sizing error. Risk fraction and deployment fraction are different quantities — confusing them causes 20× position undersizing at typical stop distances.

Introduced (corrected): Week 6. Reinforced: Week 9. Source: METHODOLOGY_STANDARDS.md, RR-BNB-005.

**Monte Carlo Bootstrap Method** — Resample actual trade returns with replacement across 1,000+ simulations at multiple win rate scenarios (or magnitude scaling for trend-following). Produces P10/P50/P90 annual return distribution, probability of negative year, and Kelly fraction at each scenario. Required for all strategies with fewer than 100 backtest trades — with small samples, the point estimate (median annual return) is unreliable; the distribution of outcomes is the relevant information. For BNB Donchian (74 trades): P(neg year) = ~0% even at 65% win rate (well below backtest 43.2%). This is the most important risk gate before deployment: if P(neg year) > 30% at any realistic win rate scenario, the strategy should not be deployed. Monte Carlo is non-negotiable per METHODOLOGY_STANDARDS.md Fat-Tail Warning section — it is the only method that uses the actual observed trade distribution without assuming normality.

Introduced: Week 6. Reinforced and applied to BNB: Week 9. Source: bnb_montecarlo_results.csv, METHODOLOGY_STANDARDS.md.

**BNB Market Characteristics** — BNB (Binance Coin) has structural price support not present in other altcoins: (1) Binance exchange fee discounts for BNB holders create persistent buy pressure proportional to Binance trading volume; (2) quarterly auto-burn mechanism permanently removes circulating supply; (3) BNB Chain (formerly BSC) gas demand creates non-speculative demand for the token. These create a different price dynamic than pure speculative altcoins — BNB tends to have stronger recovery from drawdowns because the utility demand provides a floor. Despite this, BNB suffered −70%+ in 2022 (B&H MDD −80.1% full period). The structural support did not prevent the bear market but did contribute to faster recovery. Regulatory tail risk: BNB is uniquely exposed to adverse action against Binance itself (2023 DOJ settlement caused a sharp independent drawdown). This risk is accepted at $150 capital but must be reviewed at $500+ (RR-BNB-003). Post-break Donchian PF 2.961 — regime-resilient breakout edge confirmed despite institutional adoption effects.

Introduced: Week 9. Source: RISK_REGISTER_BNB_DONCHIAN.md, bnb_exit_regime_results.csv.

**Altcoin Discovery Grid Results (Week 9)** — Multi-asset discovery grid run on LINK, DOT, MATIC, BNB, AVAX using the framework from sol_grid_search.py (5 indicator families, ~1,478 combinations, hard filters ≥30 trades + MDD > −50% + B&H benchmark gate). Results: LINK — zero passes (high volatility blows all stop distances); DOT — best Sortino 0.49 (structural multi-year downtrend eliminates trend-following edge); MATIC — B&H floor too high from 2020–2021 extreme bull run, no combination clears B&H-adjusted gates post-break; AVAX — one pass (Keltner EMA=15/mult=2.0) but post-break PF 1.413 — edge compressed, win rate declined 53%→33%; BNB — 23+ combinations passed B&H-exception-adjusted filters, 2 Donchian combos proceeded to full validation. Key learning: when an asset has a −80%+ B&H MDD (BNB, MATIC), the standard B&H benchmark gate becomes meaningless and the exception rule must be applied. When an asset is in a structural downtrend (DOT), trend-following has no edge regardless of parameter optimisation.

Introduced: Week 9. Source: altcoin_discovery_results.csv, STRATEGY_ARCHIVE.md S011–S016.

**Visualisation as Analysis Tool** — The chart package (seven interactive Plotly HTML charts) is not a documentation exercise — it is an analysis tool that reveals information not visible in summary statistics. The equity curve's log scale exposes how 2021's outlier compresses all other years visually; the underwater curve makes the difference between −29.7% (strategy) and −80.1% (B&H) visceral; the stability heatmap reveals which regions of parameter space are viable vs fragile at a glance. Charts that look similar on summary tables (e.g., two strategies with similar annual return) often look dramatically different when their equity curves and drawdown profiles are overlaid. Required as named Phase 3/4/6 deliverables per METHODOLOGY_STANDARDS.md and STRATEGY_DEPLOYMENT_TEMPLATE.md — not optional extras. Phase 6 is not complete until all seven charts are produced and verified. Interactive HTML is the deployment standard; static PNG is acceptable for quick reference only.

Introduced: Week 9. Source: 07_bnb_charts.py, METHODOLOGY_STANDARDS.md Chart Production Standards section.
