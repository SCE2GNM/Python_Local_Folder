"""
PHASE 4: CAPSTONE STRATEGY (Regime Switching - Aggressive)
...
NOW INCLUDES: Automated CSV Logging, Advanced Risk Metrics, and Professional Visualization.

THE STRATEGY LOGIC:
1. CALCULATE REGIME (The "Weather Report"):
   - We use ADX (Average Directional Index) to measure Trend Strength.
   - AGGRESSIVE LOGIC: We use a single threshold (25).
     * ADX > 25: Enter Trend Mode Immediately.
     * ADX < 25: Enter Chop Mode Immediately.
...
# Strategy Parameters
ADX_PERIOD = 14
ADX_THRESHOLD = 25      # Single Threshold (Aggressive Switch)
...
def run_capstone_strategy(df):
    ...
    for i in range(len(df)):
        ...
        # -----------------------------
        # 1. DETERMINE DESIRED REGIME (Aggressive Logic)
        # -----------------------------
        target_state = "CASH"
...
def log_to_csv(metrics, fee_total):
    ...
    Includes the 'Source_File' column for version tracking.
    """
    # Get the filename of the script being executed
    try:
        current_file = os.path.basename(__file__)
    except NameError:
        current_file = "interactive_notebook" # Fallback if run in Jupyter/interactive mode

    # Data to save
    log_data = {
        "Timestamp": pd.Timestamp.now(),
        "Source_File": current_file, # New Field: Tracks which script generated this result
        "Strategy_Desc": "Regime Switch (Aggressive)", 
